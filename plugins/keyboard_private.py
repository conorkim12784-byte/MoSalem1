...
async def get_num_for_user_and_group(m: Message):
    ...
    if get_db_checkuserall() is None:
        ...
        for per in get_db_checkuserall():
            user = user + 1
    await m.reply_video(video="https://t.me/OpsBeTa/6", caption="""
    ⌔مرحبا بك عزيزي المطور
يمكنك الان معرفه احصائياتي🔔
    """, reply_to_message_id=m.message_id, reply_markup=InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton(f"⌔ عدد الاعضاء -> {user}", url="https://t.me/Ops_BeTa")
            ],
            [
                InlineKeyboardButton(f"⌔ عدد الجروبات -> {group}", url="https://t.me/Ops_BeTa")
            ]
        ])
    )
...
async def get_information_server(m: Message):
    await m.reply_text(str(os.popen("""
        ...
        memUsedPrc=`free -m | awk 'NR==2{printf "%sMB/%sMB {%.2f%}/n", $3,$2,$3*100/$2 }'`
        HardDisk=`df -lh | awk '{if ($6 == "/") { print $3"/"$2" ~ {"$5"}" }}'`
        CPUPer=`top -b -n1 | grep "Cpu(s)" | awk '{print $2 + $4}'`
        uptime=`uptime | awk -F'( |,|:)+' '{if ($7=="min") m=$6; else {if ($7~/^day/) {d=$6;h=$8;m=$9} else {h=$6;m=$7}}} {print d+0,"days,",h+0,"hours,",m+0,"minutes."}'`
        echo '⇗ نظام التشغيل ⇖•*->-> '"$linux_version"'*' 
        echo '*———————————~*⌔✔{ الذاكره العشوائيه } ⇎*->-> '"$memUsedPrc"'*'
        echo '*———————————~*⌔✔{ وحـده الـتـخـزيـن } ⇎*->-> '"$HardDisk"'*'
        echo '*———————————~*⌔✔{ الـمــعــالــج } ⇎*->-> '"`grep -c processor /proc/cpuinfo`""Core ~ {$CPUPer%} "'*'
        echo '*———————————~*⌔✔{ الــدخــول } ⇎*->-> '`whoami`'*'
        echo '*———————————~*⌔✔{ مـده تـشغيـل الـسـيـرفـر }⇎*->-> '"$uptime"'*'
        **S O U R C E ~ H A C K E T < BY : @Alamy3 >⚡**
...
async def get_version_source(m: Message):
    ...
    await m.reply_text(f"⌔ اصدار سورس بيتا \n⌔ الاصدار -> {version}\n√", reply_to_message_id=m.message_id)
